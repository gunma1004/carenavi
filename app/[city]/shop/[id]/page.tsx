...
const shopModifiers = [
  ...
  '가벼운 방문 몸만들기', '활력 충전 웰니스', '에너지 밸런스 바디', '신선한 웰빙', '오롯한 힐링 테라피',
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const seed = `${cityName}-${id}-carenavi-city-shop-mix`;
  ...
  // 🌟 '출장'과 '마사지'가 절대 붙지 않고 중간 단어가 분산 배치되는 구조 (샵 이름 제외)
  const titleText = `${cityName} 출장 ${shopModifiers[modIdx]} ${shopActionSuffixes[srvIdx]}`;
  const descText = `${cityName} 출장 ${shopDescriptions[descIdx]}`;
...
export default async function CityShopDetailPage({ params }: PageProps) {
  ...
  const cityName = city.toUpperCase() === "SEOUL" ? "서울" : city.toUpperCase() === "GYEONGGI" ? "경기" : "인천";
  const displayShopTitle = `${cityName} 프리미엄 힐링 테라피 - ${shop.name}`;
