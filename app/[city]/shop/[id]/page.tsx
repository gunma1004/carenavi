...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const charSum = (cityName + shop.name + "carenavi_city_split").split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
  const variantIndex = charSum % 30;

  const titleVariants = [
    /* 0 */ `${cityName} 출장 타이 마사지 · 출장 아로마 마사지 - ${shop.name}`,
    /* 1 */ `${cityName} 출장 힐링 마사지 & 출장 스웨디시 테라피 · ${shop.name}`,
    /* 2 */ `[케어나비] ${cityName} 출장 타이 마사지 24시 안내 - ${shop.name}`,
    /* 3 */ `${cityName} 출장 아로마 마사지 및 출장 릴렉스 케어 | ${shop.name}`,
    /* 4 */ `${cityName} 출장 힐링 마사지 전문 가이드 - ${shop.name}`,
    /* 5 */ `${cityName} 출장 스웨디시 마사지 · 출장 타이 마사지 | ${shop.name}`,
    /* 6 */ `[안심후불] ${cityName} 출장 아로마 마사지 추천 - ${shop.name}`,
    /* 7 */ `${cityName} 출장 릴렉스 마사지 & 출장 힐링 마사지 · ${shop.name}`,
    /* 8 */ `${cityName} 출장 타이 마사지 가격 및 코스 안내 - ${shop.name}`,
    /* 9 */ `${cityName} 출장 스웨디시 마사지 및 출장 아로마 마사지 | ${shop.name}`,
    /* 10 */ `케어나비 | ${cityName} 출장 힐링 마사지 제휴 샵 - ${shop.name}`,
    /* 11 */ `${cityName} 출장 타이 마사지 · 출장 릴렉스 테라피 - ${shop.name}`,
    /* 12 */ `${cityName} 출장 아로마 마사지 24시 신속 방문 · ${shop.name}`,
    /* 13 */ `${cityName} 출장 스웨디시 마사지 & 출장 힐링 마사지 | ${shop.name}`,
    /* 14 */ `[추천 제휴] ${cityName} 출장 타이 마사지 - ${shop.name}`,
    /* 15 */ `${cityName} 출장 힐링 마사지 & 출장 아로마 마사지 · ${shop.name}`,
    /* 16 */ `${cityName} 출장 릴렉스 마사지 및 출장 스웨디시 케어 | ${shop.name}`,
    /* 17 */ `${cityName} 출장 타이 마사지 1:1 맞춤 방문 - ${shop.name}`,
    /* 18 */ `${cityName} 출장 아로마 마사지 · 출장 힐링 마사지 안내 · ${shop.name}`,
    /* 19 */ `케어나비 ${cityName} 출장 스웨디시 마사지 - ${shop.name}`,
    /* 20 */ `${cityName} 출장 힐링 마사지 및 출장 타이 마사지 | ${shop.name}`,
    /* 21 */ `${cityName} 출장 아로마 마사지 & 출장 릴렉스 테라피 - ${shop.name}`,
    /* 22 */ `[24시 후불제] ${cityName} 출장 타이 마사지 - ${shop.name}`,
    /* 23 */ `${cityName} 출장 스웨디시 마사지 · 출장 아로마 마사지 · ${shop.name}`,
    /* 24 */ `${cityName} 출장 힐링 마사지 코스별 요금표 - ${shop.name}`,
    /* 25 */ `${cityName} 출장 타이 마사지 및 출장 힐링 마사지 | ${shop.name}`,
    /* 26 */ `${cityName} 출장 아로마 마사지 전문 제휴점 - ${shop.name}`,
    /* 27 */ `케어나비 추천 ${cityName} 출장 스웨디시 마사지 - ${shop.name}`,
    /* 28 */ `${cityName} 출장 릴렉스 마사지 · 출장 타이 마사지 · ${shop.name}`,
    /* 29 */ `${cityName} 출장 힐링 마사지 최종 안내 - ${shop.name}`
  ];

  const descriptionVariants = [
    /* 0 */ `${cityName} 출장 타이 마사지, 출장 아로마 마사지 전문 ${shop.name}! 선입금 없는 100% 안심 후불제 코스 및 가격 정보를 케어나비에서 확인하세요.`,
    /* 1 */ `${cityName} 출장 힐링 마사지 및 출장 스웨디시 테라피 제휴 샵 ${shop.name}. 24시 신속 방문과 투명한 코스별 가격비교를 제공합니다.`,
    /* 2 */ `선입금 사기 걱정 없는 100% 후불제! ${cityName} 출장 타이 마사지 프로그램과 전문 관리사 맞춤 케어를 ${shop.name}에서 만나보세요.`,
    /* 3 */ `${cityName} 출장 아로마 마사지, 출장 릴렉스 케어 전문 ${shop.name}. 지친 피로를 풀어주는 1:1 맞춤 방문 힐링 서비스를 안내합니다.`,
    /* 4 */ `${cityName} 출장 힐링 마사지 24시 예약 가이드. 검증된 ${shop.name} 제휴점에서 편안하고 안심되는 휴식을 누려보세요.`,
    /* 5 */ `${cityName} 출장 스웨디시 마사지 및 출장 타이 마사지 전문점 ${shop.name}. 25분 내 신속한 방문과 정직한 후불제 시스템을 보장합니다.`,
    /* 6 */ `안심하고 이용하는 ${cityName} 출장 아로마 마사지 ${shop.name}! 선입금 0원, 100% 후불제로 쾌적한 전신 바디케어를 경험하세요.`,
    /* 7 */ `${cityName} 출장 릴렉스 마사지, 출장 힐링 마사지 특화 제휴 샵 ${shop.name}. 세심한 터치로 일상의 피로를 말끔히 비워내 드립니다.`,
    /* 8 */ `${cityName} 출장 타이 마사지 코스별 상세 요금표 안내. ${shop.name}의 투명하고 합리적인 방문 테라피 프로그램을 확인하세요.`,
    /* 9 */ `${cityName} 출장 스웨디시 마사지, 출장 아로마 마사지 우수 제휴점 ${shop.name}. 언제나 편리하게 이용할 수 있는 실시간 예약 가이드.`,
    /* 10 */ `케어나비가 엄선한 ${cityName} 출장 힐링 마사지 ${shop.name}. 100% 후불제로 안전하고 편안한 나만의 홈스파를 즐겨보세요.`,
    /* 11 */ `${cityName} 출장 타이 마사지, 출장 릴렉스 테라피 전문 ${shop.name}! 숙련된 힐러진의 정성스러운 1:1 방문 케어 안내.`,
    /* 12 */ `${cityName} 출장 아로마 마사지 24시 연중무휴 운영 ${shop.name}. 깊은 이완과 힐링을 선사하는 프리미엄 방문 테라피.`,
    /* 13 */ `${cityName} 출장 스웨디시 마사지 및 출장 힐링 마사지 제휴 정보. ${shop.name}에서 제공하는 정직한 코스별 가격을 비교하세요.`,
    /* 14 */ `신속한 방문과 친절한 서비스! ${cityName} 출장 타이 마사지 전문 ${shop.name}의 안심 후불제 프로그램을 만나보세요.`,
    /* 15 */ `${cityName} 출장 힐링 마사지, 출장 아로마 마사지 인기 제휴점 ${shop.name}. 지친 몸과 마음에 편안한 쉼을 선물해 드립니다.`,
    /* 16 */ `${cityName} 출장 릴렉스 마사지, 출장 스웨디시 케어 ${shop.name}. 선입금 없는 안전한 후불 시스템으로 부담 없이 이용하세요.`,
    /* 17 */ `${cityName} 출장 타이 마사지 1:1 맞춤 방문 솔루션 ${shop.name}. 굳은 전신 근육을 시원하게 풀어주는 전문 힐링 프로그램.`,
    /* 18 */ `${cityName} 출장 아로마 마사지 및 출장 힐링 마사지 가이드. ${shop.name}에서 투명하고 정직한 제휴 정보를 제공합니다.`,
    /* 19 */ `${cityName} 출장 스웨디시 마사지 전문 ${shop.name}! 최고급 오일 테라피와 함께 온전한 휴식을 누려보세요.`,
    /* 20 */ `${cityName} 출장 힐링 마사지, 출장 타이 마사지 제휴 안내 ${shop.name}. 24시간 언제든 빠르고 정확하게 연결해 드립니다.`,
    /* 21 */ `${cityName} 출장 아로마 마사지, 출장 릴렉스 테라피 ${shop.name}. 프라이빗 맞춤 케어로 일상의 활력을 되찾아보세요.`,
    /* 22 */ `100% 후불제로 안전한 ${cityName} 출장 타이 마사지 ${shop.name}. 출발 전 선입금을 절대 요구하지 않는 믿을 수 있는 제휴점.`,
    /* 23 */ `${cityName} 출장 스웨디시 마사지, 출장 아로마 마사지 종합 안내. ${shop.name}의 합리적인 코스 요금을 지금 확인하세요.`,
    /* 24 */ `${cityName} 출장 힐링 마사지 요금 및 예약 안내. ${shop.name}의 베테랑 테라피스트가 선사하는 고품격 방문 바디케어.`,
    /* 25 */ `${cityName} 출장 타이 마사지 및 출장 힐링 마사지 우수 제휴 샵 ${shop.name}. 신속한 방문 배차로 만족도를 높여드립니다.`,
    /* 26 */ `${cityName} 출장 아로마 마사지 전문 관리사 배정 ${shop.name}. 부드러운 림프 순환과 힐링을 위한 최적의 선택.`,
    /* 27 */ `케어나비 공식 ${cityName} 출장 스웨디시 마사지 ${shop.name}. 쾌적하고 편안한 휴식 공간 제휴 정보를 전해드립니다.`,
    /* 28 */ `${cityName} 출장 릴렉스 마사지, 출장 타이 마사지 제휴 프로그램. ${shop.name}과 함께 묵은 피로를 시원하게 날려버리세요.`,
    /* 29 */ `${cityName} 출장 힐링 마사지 최종 이용 가이드 ${shop.name}. 100% 안심 후불제 시스템으로 편안하고 안전하게 이용하세요.`
  ];
...
export default async function CityShopDetailPage({ params }: PageProps) {
  ...
  const cityName = city.toUpperCase() === "SEOUL" ? "서울" : city.toUpperCase() === "GYEONGGI" ? "경기" : "인천";
  const displayShopTitle = `${cityName} 출장 타이 마사지 · 출장 아로마 마사지 - ${shop.name}`;
