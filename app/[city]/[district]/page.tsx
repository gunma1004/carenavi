...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const charSum = (locationKeyword + "carenavi_district_clean").split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
  ...
  const descriptionVariants = [
    ...
    /* 29 */ `${locationKeyword} 최종 웰니스 마사지 테라피 제휴 가이드. 케어나비가 보증하는 안전하고 쾌적한 휴식 공간 정보.`
  ];
