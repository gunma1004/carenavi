...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const titleVariants = [
    /* 0 */ `${locationPrefix} 전문 출장 타이 마사지 - 케어나비`,
    /* 1 */ `${locationPrefix} 전문 출장 아로마 마사지 - 케어나비`,
    /* 2 */ `${locationPrefix} 전문 출장 스웨디시 마사지 · 케어나비`,
    /* 3 */ `[케어나비] ${locationPrefix} 24시 출장 타이 마사지 제휴`,
    /* 4 */ `${locationPrefix} 맞춤 출장 아로마 마사지 및 릴렉스 케어 | 케어나비`,
    /* 5 */ `${locationPrefix} 전문 출장 스웨디시 마사지 가이드 - 케어나비`,
    /* 6 */ `[안심후불] ${locationPrefix} 추천 출장 타이 마사지 - 케어나비`,
    /* 7 */ `${locationPrefix} 맞춤 출장 아로마 마사지 & 힐링 케어 · 케어나비`,
    /* 8 */ `${locationPrefix} 전문 출장 스웨디시 마사지 가격 안내 - 케어나비`,
    /* 9 */ `케어나비 | ${locationPrefix} 신속 출장 타이 마사지 샵`,
    /* 10 */ `${locationPrefix} 24시 출장 아로마 마사지 신속 방문 · 케어나비`,
    /* 11 */ `${locationPrefix} 프리미엄 출장 스웨디시 마사지 정보 - 케어나비`,
    /* 12 */ `[추천 제휴] ${locationPrefix} 전문 출장 타이 마사지 - 케어나비`,
    /* 13 */ `${locationPrefix} 1:1 맞춤 출장 아로마 마사지 - 케어나비`,
    /* 14 */ `케어나비 ${locationPrefix} 프리미엄 출장 스웨디시 마사지`,
    /* 15 */ `${locationPrefix} 전문 출장 타이 마사지 코스별 요금표 - 케어나비`,
    /* 16 */ `${locationPrefix} 우수 제휴 출장 아로마 마사지점 - 케어나비`,
    /* 17 */ `케어나비 추천 ${locationPrefix} 맞춤 출장 스웨디시 마사지`,
    /* 18 */ `[24시 후불제] ${locationPrefix} 전문 출장 타이 마사지 - 케어나비`,
    /* 19 */ `${locationPrefix} 최종 출장 아로마 마사지 안내 - 케어나비`,
    /* 20 */ `${locationPrefix} 안심 출장 타이 마사지 안내 - 케어나비`,
    /* 21 */ `${locationPrefix} 쾌적한 출장 아로마 마사지 안내 - 케어나비`,
    /* 22 */ `${locationPrefix} 품격 있는 출장 스웨디시 마사지 안내 - 케어나비`,
    /* 23 */ `케어나비 ${locationPrefix} 베테랑 출장 타이 마사지`,
    /* 24 */ `케어나비 ${locationPrefix} 힐링 출장 아로마 마사지`,
    /* 25 */ `${locationPrefix} 지역 맞춤 출장 타이 마사지 샵 - 케어나비`,
    /* 26 */ `${locationPrefix} 지역 맞춤 출장 아로마 마사지 샵 - 케어나비`,
    /* 27 */ `${locationPrefix} 하이엔드 출장 타이 마사지 - 케어나비`,
    /* 28 */ `${locationPrefix} 하이엔드 출장 아로마 마사지 - 케어나비`,
    /* 29 */ `${locationPrefix} 신뢰할 수 있는 출장 타이 마사지 - 케어나비`
  ];

  const descriptionVariants = [
    /* 0 */ `${locationPrefix} 전문 출장 타이 마사지 제휴처 ${shop.name}. 선입금 없는 100% 안심 후불제 코스 및 가격 정보를 케어나비에서 확인하세요.`,
    /* 1 */ `${locationPrefix} 맞춤 출장 아로마 마사지 제휴 샵 ${shop.name}. 24시 신속 방문과 투명한 코스별 가격비교를 제공합니다.`,
    /* 2 */ `선입금 사기 걱정 없는 100% 후불제! ${locationPrefix} 프리미엄 출장 스웨디시 마사지 프로그램과 맞춤 케어를 ${shop.name}에서 만나보세요.`,
    /* 3 */ `${locationPrefix} 릴렉스 케어 전문 ${shop.name}. 지친 피로를 풀어주는 1:1 맞춤 출장 마사지 서비스를 안내합니다.`,
    /* 4 */ `${locationPrefix} 24시 출장 타이 마사지 예약 가이드. 검증된 ${shop.name} 제휴점에서 편안하고 안심되는 휴식을 누려보세요.`,
    /* 5 */ `${locationPrefix} 전문 출장 아로마 마사지 점 ${shop.name}. 25분 내 신속한 방문과 정직한 후불제 시스템을 보장합니다.`,
    /* 6 */ `안심하고 이용하는 ${locationPrefix} 우수 출장 스웨디시 마사지 ${shop.name}! 선입금 0원, 100% 후불제로 쾌적한 전신 바디케어를 경험하세요.`,
    /* 7 */ `${locationPrefix} 특화 제휴 샵 ${shop.name}. 세심한 터치로 일상의 피로를 말끔히 비워내 드리는 출장 마사지 서비스.`,
    /* 8 */ `${locationPrefix} 전문 출장 타이 마사지 코스별 상세 요금표 안내. ${shop.name}의 투명하고 합리적인 방문 테라피 프로그램을 확인하세요.`,
    /* 9 */ `${locationPrefix} 실속 있는 출장 아로마 마사지 우수 제휴점 ${shop.name}. 언제나 편리하게 이용할 수 있는 실시간 예약 가이드.`,
    /* 10 */ `케어나비가 엄선한 ${locationPrefix} 안전 출장 스웨디시 마사지 ${shop.name}. 100% 후불제로 안전하고 편안한 나만의 홈스파를 즐겨보세요.`,
    /* 11 */ `${locationPrefix} 전문 ${shop.name}! 숙련된 힐러진의 정성스러운 1:1 방문 출장 마사지 케어 안내.`,
    /* 12 */ `${locationPrefix} 24시 연중무휴 운영 ${shop.name}. 깊은 이완과 힐링을 선사하는 고품격 방문 출장 마사지 프로그램.`,
    /* 13 */ `${locationPrefix} 투명한 출장 아로마 마사지 제휴 정보. ${shop.name}에서 제공하는 정직한 코스별 가격을 비교하세요.`,
    /* 14 */ `신속한 방문과 친절한 서비스! ${locationPrefix} 전문 출장 스웨디시 마사지 ${shop.name}의 안심 후불제 프로그램을 만나보세요.`,
    /* 15 */ `${locationPrefix} 인기 제휴점 ${shop.name}. 지친 몸과 마음에 편안한 쉼을 선물해 드리는 출장 마사지 힐링 스팟.`,
    /* 16 */ `${locationPrefix} 선입금 없는 안전한 후불 시스템으로 부담 없이 이용하는 출장 타이 마사지 ${shop.name}.`,
    /* 17 */ `${locationPrefix} 맞춤형 출장 아로마 마사지 솔루션 ${shop.name}. 굳은 전신 근육을 시원하게 풀어주는 전문 방문 테라피.`,
    /* 18 */ `${locationPrefix} 투명하고 정직한 제휴 정보를 제공하는 출장 스웨디시 마사지 가이드 ${shop.name}.`,
    /* 19 */ `${locationPrefix} 전문 ${shop.name}! 최고급 오일 테라피와 함께 온전한 출장 마사지 휴식을 누려보세요.`,
    /* 20 */ `${locationPrefix} 24시간 언제든 빠르고 정확하게 연결되는 출장 타이 마사지 제휴 안내 ${shop.name}.`,
    /* 21 */ `${locationPrefix} 프라이빗 맞춤 케어로 일상의 활력을 되찾아주는 출장 아로마 마사지 제휴 샵 ${shop.name}.`,
    /* 22 */ `100% 후불제로 안전한 ${locationPrefix} 출장 스웨디시 마사지 ${shop.name}. 출발 전 선입금을 요구하지 않는 믿을 수 있는 제휴점.`,
    /* 23 */ `${locationPrefix} 합리적인 출장 타이 마사지 코스 요금을 ${shop.name}에서 지금 확인하세요.`,
    /* 24 */ `${locationPrefix} 베테랑 테라피스트가 선사하는 고품격 출장 아로마 마사지 요금 및 예약 안내 ${shop.name}.`,
    /* 25 */ `${locationPrefix} 신속한 방문 배차로 만족도를 높여주는 출장 스웨디시 마사지 우수 제휴 샵 ${shop.name}.`,
    /* 26 */ `${locationPrefix} 전문 관리사 배정 ${shop.name}. 부드러운 림프 순환과 출장 마사지 힐링을 위한 최적의 선택.`,
    /* 27 */ `케어나비 공식 ${locationPrefix} 출장 타이 마사지 ${shop.name}. 쾌적하고 편안한 방문 휴식 공간 제휴 정보를 전해드립니다.`,
    /* 28 */ `${locationPrefix} 묵은 피로를 시원하게 날려버리는 출장 아로마 마사지 제휴 프로그램 ${shop.name}.`,
    /* 29 */ `${locationPrefix} 최종 이용 가이드 ${shop.name}. 100% 안심 후불제 시스템으로 편안하고 안전하게 즐기는 출장 마사지.`
  ];
...
export default async function DistrictShopDetailPage({ params }: PageProps) {
  ...
  const cityText = city.toUpperCase() === "SEOUL" ? "서울" : city.toUpperCase() === "GYEONGGI" ? "경기" : "인천";
  const locationPrefix = `${cityText} ${district}`;
  const displayShopTitle = `${locationPrefix} 전문 출장 타이 마사지 - ${shop.name}`;
